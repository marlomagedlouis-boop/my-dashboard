            ...
            const renderPlatformRow = (platform) => {
                ...
                if (platform === 'Customer Portal' && coreKpis.includes(kpi) && platformData.length > 1) {
                    const worstChannel = platformData.map(d => {
                        // For cumulative, compare MTD actual against full month target to determine gap
                        const targetForGap = isCumulative ? (parseFloat(d.Full_Month_Target) || 0) : (parseFloat(d.Full_Month_Target) || 0) * (daysPassed / totalDays);
                        return { name: d.Channel, gap: targetForGap > 0 ? ((parseFloat(d.MTD_Actual) || 0) / targetForGap) - 1 : 0 };
                ...
                // FIX: Logic for displaying targets for cumulative vs. prorated KPIs
                const cells = (() => {
                    if (isCumulative) {
                        const achievement = fullTarget > 0 ? (actual / fullTarget) * 100 : 0;
                        return `<td class="value-cell">${formatValue(kpi, fullTarget)}</td>
                                <td><div class="pacing-cell"><span class="performance-text">${achievement.toFixed(0)}% of Full Target</span></div></td>`;
                    } else {
        ...
        try {
            ...
            if (type === 'monthly') {
                ...
                channels.forEach(ch => {
                    ...
                    if (!isNonTarget) {
                        const fullMonthTarget = parseFloat(entry?.Full_Month_Target) || 0;
                        // FIX: Use full month target for cumulative KPIs in the weekly drilldown as directed
                        const targetValue = isCumulative ? fullMonthTarget : fullMonthTarget * multiplier;
            ...
            const chartConfig = {
                ...
                options: {
                    ...
                    plugins: { 
                        legend: { display: !isNonTarget }, 
                        datalabels: { anchor: 'end', align: 'end', formatter: (value) => formatValue(kpi, value), color: '#444' }
                    },
                    scales: { y: { beginAtZero: true } }
                }
            };

            if (type === 'weekly' && !isNonTarget) {
                chartConfig.options.plugins.datalabels = {
                    ...
                    color: (context) => {
                        const targetValue = context.chart.data.datasets[1].data[context.dataIndex];
                        const percentage = targetValue > 0 ? (context.dataset.data[context.dataIndex] / targetValue) : 0;
                        // For cumulative, the percentage is achievement. For others, it's pacing.
                        const threshold = isCumulative ? 0.9 * (daysPassed / totalDays) : 0.9;
                        return percentage >= 1 ? '#28a745' : (percentage >= threshold ? '#ffc107' : '#d93025');
                    },
                    anchor: 'end',
                    align: 'end'
                };
